...
- **Потоковая передача видео** с поддержкой перемотки

## Ключевые особенности

| Функция | Описание | Преимущество |
|---------|----------|--------------|
| Async API | Полностью асинхронный бэкенд на FastAPI + asyncpg | Высокая производительность, ~1000 RPS |
| Video Streaming | HTTP Range Requests для потокового видео | Мгновенная перемотка, экономия трафика |
| JWT Auth | Access + Refresh токены | Безопасная аутентификация |
| Чат комментариев | Обсуждение ответов преподаватель-студент | Быстрая обратная связь |
| Журнал оценок | Сводная таблица успеваемости группы | Удобный контроль прогресса |
| Docker Ready | Полная контейнеризация с Nginx | Деплой в один клик |
...
- **Repository Pattern** — CRUD-слой абстрагирует работу с БД
- **Dependency Injection** — FastAPI Depends для авторизации и сессий
- **DTO/Schema Separation** — Pydantic схемы отделены от SQLAlchemy моделей
- **Async/Await** — неблокирующие операции с БД и файлами

## Потоковое видео (Video Streaming)

Платформа поддерживает **HTTP Range Requests** для эффективной потоковой передачи видео:

- **Мгновенная перемотка** — пользователь может переходить к любому моменту видео без ожидания полной загрузки
- **Экономия трафика** — загружается только просматриваемый фрагмент (чанки по 2 МБ)
- **Поддержка больших файлов** — видео до 100 МБ воспроизводятся без задержек
- **HTTP 206 Partial Content** — стандартный протокол, совместимый со всеми браузерами

```
GET /api/v1/files/stream/lessons/{course_id}/{lesson_id}
Range: bytes=0-2097151

HTTP/1.1 206 Partial Content
Content-Range: bytes 0-2097151/50000000
Accept-Ranges: bytes
```

Это позволяет студентам сразу начинать просмотр и свободно перематывать видеолекции.
